...
- **skill-skills-upstream-sync.AC6.6 Success** (reframed M6 revision 2026-04-18; design plan corrected 2026-07-07 per Phase 6 coherence-review M1 — this AC originally described an authoring-time *rejection gate that blocks ND*, which the M6 revision superseded but the design plan had not captured; verdict enumeration extended to pass/fail/**split** 2026-07-07 per coherence-gate-close Minor-1, reflecting the M5 mixed-signal exception the self-audit emits at SKILL.md:918): `impl-plan-write/SKILL.md` Task ND (per-phase file write) is preceded by a **non-blocking pre-presentation self-audit** (step 6.5): before the phase file is written, the planner runs the three anti-smuggling tests on each proposed UAT entry and surfaces pass/fail/**split** with suggested re-routing (to test-requirement, deferred-to-future-phase, split-and-keep-the-residual, or "no UAT entry for this decision") so the step-7 user approval is informed. It does NOT block ND — the user can still approve a surfaced entry. The **structural** authoring-time gate is the collation audit (AC6.2), an independent subagent that scores every entry before `uat-requirements.md` is written. Both enforce at authoring, not at exec-uat-gate execution (too late per the parallel-session finding: "The three tests live only in exec-uat-gate, which fires at execution — too late")
