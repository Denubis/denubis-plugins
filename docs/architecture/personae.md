...
6. **Auditing auto-memory drift.** Invokes `denubis-dream:dreaming` via `/dream` to surface stale memories (claims that no longer hold, mention deleted files, or duplicate other memories) and memory-worthy untracked transcript regions. The plugin's autonomous pass dispatches parallel Sonnet evidence subagents and an Opus judgement pass, producing a dated scratch tree under `~/.claude/projects/<main-slug>/memory.dream-YYYY-MM-DD/`. The user then walks the audit memory-by-memory (oldest first), accepting/editing/pruning existing memories and promoting flagged transcript regions into new memories before atomic finalisation. Can run unattended on cron via the `schedule` skill invoking `/dream --autonomous` (`docs/design-plans/2026-05-16-denubis-dream.md`, `3a26c87`).
