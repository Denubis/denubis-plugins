...
- **Liveness file** (planned): a key=value text file at `~/.claude/run/<wrapper-pid>.live` that the patched `claude-wrapper.sh` writes on start and removes on clean exit. A surviving file after the wrapper process disappears signals that the session ended uncleanly. Contains `cwd`, `started`, `argv`, and `boot_id` lines (`docs/design-plans/2026-05-08-crash-recovery.md`, uncommitted).
- **boot_id** (planned): the kernel-assigned identifier read from `/proc/sys/kernel/random/boot_id`; regenerates on every system boot. Written into the liveness file so that `crash-recovery scan` can recognise liveness files orphaned by a reboot regardless of whether their recorded PID is currently alive (`docs/design-plans/2026-05-08-crash-recovery.md`, uncommitted).
- **classifier_version** (planned): an integer constant in crash-recovery's classification rule table, stored on every `sessions` row and on every `scan_runs` row. When the rule table changes, scan re-classifies any rows whose stored value is below the current constant before render or prune sees them (`docs/design-plans/2026-05-08-crash-recovery.md`, uncommitted).
...
| WAL | Write-Ahead Logging | SQLite concurrency mode planned for crash-recovery's source-of-truth database (`docs/design-plans/2026-05-08-crash-recovery.md`, uncommitted) |
