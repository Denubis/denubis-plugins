# denubis-bibliography — Context (Level 0)

> System boundary: a single-skill plugin that renders Zotero-managed PDFs to per-page markdown under `~/zettelkasten/papers/<citekey>/` and emits page-keyed blockquotes for use as verified citations. The skill's own frontmatter and manifest mark this plugin as WIP — only the workflow validated end-to-end on one paper is documented.
...
| Zotero + Better BibTeX | Local Zotero application with the Better BibTeX plugin, reachable via JSON-RPC at `http://localhost:23119`. The skill explicitly *does not* fetch papers itself — the user adds them via the Zotero connector (`plugins/denubis-bibliography/skills/using-bibliography/SKILL.md`, `18f3b80`). | Connector ping (`/connector/ping`); citekey-to-attachment lookups | PDF attachment paths + bibliographic metadata |
...
- The single skill `using-bibliography` and its three helper scripts:
  - `ingest.py` — accepts a list of DOIs / citekeys and drives the per-paper render pipeline (`plugins/denubis-bibliography/skills/using-bibliography/ingest.py`, `18f3b80`).
...
### Skills (`plugins/denubis-bibliography/skills/`)
...
- **Plugin manifest:** `plugins/denubis-bibliography/.claude-plugin/plugin.json` (`18f3b80`), version 0.1.0. Manifest description: *"Render PDFs from a Zotero corpus to per-page markdown for engagement; emit page-keyed blockquotes with pandoc-style citations. WIP — only documents the workflow proven end-to-end so far."*
