...
| `~/.claude/run/<pid>.live` files | Per-wrapper-PID liveness markers. Key=value lines: `cwd`, `started`, `argv`, `boot_id`, plus `session_id` and `start_time` (shipped Phase 2) for exact correlation and PID-reuse-proof liveness. The wrapper bootstraps `session_id` at launch; the `update-live-marker.py` SessionStart hook then keeps it on the *live* transcript across `/clear` rotation (ADR 0003), so the exact match resolves the session the operator is actually in (`docs/design-plans/2026-06-12-crash-detection.md`, DR4; `docs/architecture/decisions/0003-marker-tracks-live-transcript-via-sessionstart-hook.md`). Atomic via tempfile + `mv` (`rename(2)`). Absence after abnormal exit is the crash signal. | (none — this is a Filesystem location the plugin reads) | (none) |
| `~/.claude/projects/*/<uuid>.jsonl` | Claude Code's per-session transcript files. The plugin walks these to extract session UUIDs, the `cwd` (via a bounded forward scan to the first record carrying it — transcripts now open with a snapshot record, so line 1 alone is insufficient; `docs/design-plans/2026-06-12-crash-detection.md`, DR3), and the tail kind for classification. | (none — Filesystem read) | (none) |
| `~/.byobu-sessions/tmux_resurrect_*.txt` | tmux-resurrect snapshots (shipped Phase 3). Stage-2 correlation reads the latest snapshot at/before a marker's `started` to corroborate id-less backlog markers **by pane cwd (path-based)**; `label_for_cwd` uses the `✳` window-title slug only to pick a render label (`docs/design-plans/2026-06-12-crash-detection.md`, Phase 3). | (none — Filesystem read) | (none) |
...
- **Correlation join — forward-cwd repair + Stage-1 `session_id` + Stage-2 backlog matching all shipped.** The line-1-only cwd read that made `correlate()` return `no_match` for 33/36 markers is **repaired**: a bounded forward scan finds the first cwd-bearing record past the snapshot prefix, and the scan is dedup-safe so `triage` survives once markers correlate (both Phase 1, shipped). **Stage-1 exact `session_id` match is shipped** (Phase 2): the wrapper stamps `session_id`, the `update-live-marker.py` SessionStart hook keeps it on the live transcript across `/clear` (ADR 0003), and `correlate.py` "branch 0" resolves it as the highest-confidence match ahead of argv `--resume`. **Stage-2 backlog matching is shipped** (Phase 3): a tight first-entry-ts window `[started-60, started+120]` plus tmux-resurrect pane corroboration (path-based, by each candidate's own cwd) resolve id-less backlog markers; residual-ambiguous markers list all candidates and never silently pick. (`docs/design-plans/2026-06-12-crash-detection.md`, DR1–DR3; `jsonl.py::first_record_field`, `scan.py::_walk_sessions`, `correlate.py::_jsonl_first_entry_ts_in_tight_window` + corroboration block, `resurrect.py`.)
...
- **Marker-less retroactive recovery.** Sessions that ran before the wrapper was installed have *no* liveness file at all; they cannot be classified as `hard_crash` and remain out of scope (`docs/design-plans/2026-06-12-crash-detection.md`, DR9). Recovery via mtime-clustering against `last -F`/`journalctl` for the marker-less case is a consciously deferred future extension. (Distinct from the *id-less backlog* — markers the wrapper already wrote but without a `session_id` — which **is** now in scope via Stage-2 correlation; see "In scope" below.)
...
| `CRASH_RECOVERY_RESURRECT_DIR` | `~/.byobu-sessions/` | tmux-resurrect snapshot directory for Stage-2 corroboration (shipped Phase 3; wired into `scan`/`triage`/`regenerate`). |
