...
> System boundary: a knowledge-skill plugin shipping nine `SKILL.md` files plus one agent that together describe how to author, test, and maintain Claude Code plugins (skills, agents, marketplaces, CLAUDE.md files, upstream syncs, project context).
...
### Skills (`plugins/denubis-extending-claude/skills/`, 9 total)
...
Skill-adjacent files: `writing-claude-directives/graphviz-conventions.dot` and `writing-claude-directives/long-running-state-patterns.md` are reference assets the skill points to (`2a3f5ff`).
