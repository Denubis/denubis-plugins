...
**Edit-path exercise obligation (2026-07-05, Phase 4 code-review gate).** The Phase 4 review flagged that the edit-re-entry rule (`f88007c`) has no pressure scenario of its own — the 4/4 GREEN was creation-shaped only. The operator accepted this as non-blocking for Phase 4 closure on one condition: this work item is the edit path's first real exercise. When it edits the tested sub-skills it MUST observe and record whether the edit-re-entry rule actually fires — rubric re-check triggered, weakened pressure scenarios re-run — rather than asserting compliance. If the rule does not fire in practice, that is a finding to surface, not to paper over. This converts the Phase 4 named limitation ("Creation-shaped evidence only", `phase_04_green_verification.md`) into a bound obligation on this task.

**Boundary-condition specification (2026-07-05 proleptic CA1).** The obligation is bound to a *test design*, not merely "an edit will happen." The scenario that discharges it must exercise a boundary the edit-re-entry rule exists to catch — e.g. an edit that *should* trigger re-entry into the three-sub-skill sequence but is easy to skip under time/scope pressure, or one where the rule's trigger condition ("an edit that changes a skill's scope") is ambiguous at the margin. An incidental edit that never approaches the rule's trigger does **not** discharge the obligation, however green it leaves the tree. Marking it discharged requires a scenario built to falsify the rule, and a recorded pass/fail against that scenario.

## 2026-07-07 — cross-link: impl-plan-write structural-refactor items deferred here from Phase 6 §3d

The Phase 6 §3d refactor pass (Fable read-only prose-structure review of `impl-plan-write/SKILL.md`) surfaced two Major structural smells — **M1** (per-phase steps 1–3 triplicated ×3 across the interactive/batch/design-decisions branches) and **M2** (the design-decisions doctrine block, 675–816, lacks structural subheadings) — plus a Minor **m3** (stamp template duplicated in two sites). These are **structural refactors of impl-plan-write**: a different concern and a different file from this task's obra-import into `writing-claude-directives` / `testing-skills-with-subagents`, so they are **not** merged into this work item. They are recorded with full dispositions in `refactor-pipeline-findings-phase-6-3d.md` as a distinct "impl-plan-write structural refactor" item. Shared discipline if both are worked in one sitting: M2's heading promotion sits adjacent to test-locked wording (`SKILL.md:730–742`) and must re-run the E1–E12 fixture before shipping, exactly as this task's edits to the tested sub-skills must (edit-path obligation above). Three outright residuals from the same §3d pass were fixed immediately in commit `9cad6da`, not deferred.
