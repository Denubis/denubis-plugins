...
- **AskUserQuestion satisfaction path coverage** (deferred from Phase 2 proleptic challenge CA3, 2026-05-16) — `parse_tail` has two AskUserQuestion satisfaction branches: `_has_matching_tool_result` (generic `tool_use_id` match in a `tool_result` content item) and `_has_ask_question_answer` (`toolUseResult.answers` populated). The `toolUseResult.answers` path is fixture-tested; the `tool_result` path for AskUserQuestion specifically is only exercised indirectly via the Bash interleave fixture. During Phase 4's investigator pass (real-JSONL sampling), check whether any real AskUserQuestion session is satisfied via `tool_result` rather than `toolUseResult.answers`. If yes: add a dedicated fixture and test. If no (always-answers in real sessions): mark `_has_matching_tool_result`'s AskUserQuestion branch as dead-on-AskUserQuestion-but-needed-on-other-tools in a comment.
