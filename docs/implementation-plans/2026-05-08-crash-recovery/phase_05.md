...
**Standard entry** (any classification except `irrecoverable`):
...
**Irrecoverable entry** (`classification == "irrecoverable"`, which covers both Phase 2's `missing_jsonl_on_disk` and Phase 4's M3 `missing_cwd`):

```
- **<uuid-short>**: ~~no resume — <reason>~~
  - Working dir: `<cwd>`
  - Classification: `<classification>` (`<reason>`)
  - State: <state_summary>
  - Notes: <user_notes>                              ← only when user_notes is not NULL
...
The strikethrough resume marker prevents the misleading "try to resume this!" surface for sessions where resume is structurally impossible — there is no JSONL on disk, or there is no cwd recorded so `claudew --resume` would land in the wrong working directory. The reduced-confidence line is omitted because `_reduced_confidence_text` does not fire for `missing_jsonl_on_disk` / `missing_cwd` anyway; their irrecoverability is the strongest signal already. (Falsification anchor recorded after Phase 4 coherence review, 2026-05-17: Phase 5 must not emit `claudew --resume` commands for sessions with `cwd=""` or `classification=irrecoverable`.)
...
`_render_entry(row) -> list[str]` produces the per-entry lines exactly as the template specifies. It branches on `classification`: irrecoverable rows emit the strikethrough resume marker (`~~no resume — <reason>~~`) and skip the reduced-confidence line; every other classification emits the standard `claudew --resume <full-uuid>` opening line.
...
- **`test_irrecoverable_row_suppresses_resume_command`** — render a fixture with one `irrecoverable / missing_jsonl_on_disk` row and one `irrecoverable / missing_cwd` row (Phase 4's M3 outcome with `cwd=""`). For each, assert the rendered entry contains `~~no resume — missing_jsonl_on_disk~~` or `~~no resume — missing_cwd~~` and does NOT contain the substring `claudew --resume`. Assert the entry's other lines (Working dir, Classification, State) still appear. Locks the falsification anchor: irrecoverable rows must not advertise a resume command. The `expected_mixed.md` snapshot in `test_render_matches_snapshot[mixed]` MUST include the irrecoverable row in this form (the snapshot is the byte-level contract).
