...
| #11 | M4 — testpaths widening "acceptable" without verification (test FAILED) | ~~Reverse the widening entirely (option 4b); per-plugin invocation convention.~~ **SUPERSEDED 2026-05-15 by commit `56bd7cd`.** The 4b decision rested on the empirically-false premise that `workflow_statusline` already followed a working per-plugin convention from the worktree root — `uv run --project <plugin> pytest -q` does NOT collect the plugin's tests (pytest finds the root `pyproject.toml` first; `--project` only scopes uv's deps, not pytest's rootdir). Resolution adopted: **uv workspace + remove the root `conftest.py` `collect_ignore_glob` + widen testpaths**. One `uv run pytest -q` from the worktree root now collects 615 tests (457 + 140 + 18). Install model preserved (per-plugin `pyproject.toml` still standalone). | phase_01.md, test-requirements.md, design plan — all swept in next commit |
