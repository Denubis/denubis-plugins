...
- **(Phase 7 verification flake, 2026-05-19)**: `test_classification_history_appends_when_classification_changes` in `plugins/denubis-crash-recovery/scripts/crash_recovery/tests/test_scan.py` failed once during a wider-suite run in Phase 7 verification, then passed in isolation and on retry of the full suite. The test uses `tmp_path` (pytest tmpdir), not the production DB, so it is not state pollution. Likely a test-ordering / fixture-interaction issue. The Phase 8 finalization code review should run the Phase 2 test suite with deterministic ordering (e.g. `pytest -p no:randomly`) and chase the flakiness if it re-surfaces. Not a Phase 7 scope item — flagged for Phase 8 awareness so it does not get lost in the commit-log noise.
- **(Phase 7 coherence M3, deferred to Phase 8 Finalization)**: `docs/architecture/plugins/denubis-crash-recovery/0-context.md` does not exist despite `denubis-crash-recovery` shipping a user-facing surface as of Phase 7. The architecture README ("Plugin Contexts (14)") should bump to 15 once the file lands. Defer to Phase 8 Finalization so the plugin-context docs land together with the wrapper-touching `denubis-plan-and-execute` plugin context. Author the file as a thin overview of: purpose, public surface (CLI subcommands + triage skill), key modules, environment variables, dependencies (sibling plugin wrapper), and pointers to design/implementation plans.
- **(Phase 7 coherence L5, fitness-function for Phase 8 bats)**: `tests/test_crash_recovery_smoke.bats` asserts `[ "$status" -ne 0 ]` for the `prune-without-confirm-refuses` test. The CLI exit-code contract pins this at exit 1 specifically (the "no data / refusal" class). Tightening to `[ "$status" -eq 1 ]` would catch regressions where a future refactor maps the refusal to exit 2 or 0. Fold into the Phase 8 bats lifecycle work or test-requirements.md regeneration.
- **(Phase 7 coherence L6, fitness-function for Phase 8 bats)**: AC8.1 (README documents the sibling-plugin dependency) is currently only verified by reviewer inspection. A bats assertion — e.g. `grep -q 'denubis-plan-and-execute' plugins/denubis-crash-recovery/README.md` — would lock the dependency-text presence as an automated regression guard. Fold into the Phase 8 bats lifecycle work or test-requirements.md regeneration.
...
1. **Substitute `<PHASE-8-VERSION>` placeholder.** Phase 7 already fixed the README:20-32 and SKILL.md:99 overclaims inline (the "every session classified `concluded`" and "degrades to JSONL-tail-only heuristics" framings, replaced with the honest "`hard_crash` and `live` cannot fire without a liveness file" framing — commits TBD in the Phase 7 wrap). Phase 8's remaining doc work is substituting `<PHASE-8-VERSION>` in `plugins/denubis-crash-recovery/README.md` with the real wrapper version when both plugins bump (e.g. `2.32.2`). Verify the substitution caught with `grep -n 'PHASE-8-VERSION' plugins/denubis-crash-recovery/`.

2. **Extend the README Troubleshooting section with wrapper-side failure modes.** Phase 7's Troubleshooting covers reader-side failures only (Linux-only, atomic-rename refusal, env-var-misconfig, prune regret, schema corruption). Phase 8 introduces the *writer* — add entries for: (a) liveness-file write permission errors (`~/.claude/run/` not writable by the user); (b) wrapper crashed before atomic-rename so a temp file is orphaned; (c) writer/reader on different filesystems (NFS-mounted home with local-only `/run`, or the reverse); (d) `boot_id=unknown` in a liveness file written before `/proc/sys/kernel/random/boot_id` was readable. Each entry pairs the symptom with a one-line remediation.

3. **Do NOT implement post-mortem crash detection in Phase 8.** The design seed has 8 open questions that need brainstorming first. Scope this phase strictly to the wrapper patch + version coordination + the two doc items above.

4. **Flag the design seed at completion.** When Phase 8 wraps and the implementation plan is closed, the design seed becomes the input to the next brainstorming session. The Finalization step (post-Phase-8 code-reviewer pass over all eight phases) should explicitly call out the design seed as a known follow-up, not as an oversight.
