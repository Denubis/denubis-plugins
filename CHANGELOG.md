...
- `fable-advisor` agent, so a consultation can be dispatched as a subagent instead of spawned in a pane. A `tools:` allowlist denies what it omits, MCP tools included, which replaces the pane's 36-name deny list with a frontmatter field, and the dispatch returns a completion notification the pane never had. Permitted by a dated operator ruling (2026-07-25) recorded in `model-tier-notes.md`, which narrows the Fable cost gate rather than revoking it: nothing may name the advisor as a step, the tool surface is read-only, and dispatch still requires the human to have asked, though a model may suggest that they do. `test_no_agent_definition_declares_a_fable_model` became `test_fable_agent_definitions_carry_the_human_trigger_constraint`, asserting what such a definition must contain rather than forbidding it outright; both breach shapes were injected and watched fail.
...
**Changed:**
- The advisor's documented default is the pane variant, which is the only path whose restriction is real and the one that keeps `Glob` and `Grep` for the grounding work. Its cost is that it finishes silently, so the skill says to arm a monitor at spawn. The dispatched background agent is documented as a convenience for advisors reading something they cannot damage, with its surface enumerated from an executed probe rather than from the tool schema: `Write` and `Edit` are absent, but `Workflow`, `CronCreate`, `ScheduleWakeup`, `RemoteTrigger` and the three generic MCP resource tools are all live, and `Glob` and `Grep` are not. On that path "advises, never implements" is a brief it is asked to honour rather than a property the harness enforces. Evidence in `docs/audits/2026-07-25-fable-dispatch-surface-probe.md`.
...
**Fixed:**
- `fable-advisor-spawn.sh` prints a transcript path that resolves. The slug it suggested replaced only `/`, while the real directory name maps both `/` and `.` to `-`, so the documented way to read a pane advisor's reply returned nothing for every path under a dotted directory, which is every worktree in this repository.
- The spawned pane carries a `@fable_advisor` tmux option, so a monitor can identify it. The process name cannot: the launch reports `claude`, and so does every other Claude session in the window. The header now also records, with the measurement behind it, that the multi-argument form is exec'd directly by tmux and must not be "fixed" with a shell `exec` prefix.
