#!/bin/bash
# dispatcher-priority: 50
# RTK auto-rewrite hook for Claude Code PreToolUse:Bash
# Transparently rewrites raw commands to their rtk equivalents.
# Outputs JSON with updatedInput to modify the command before execution.
#
# IMPORTANT: "uv run <tool>" commands must rewrite to "uv run rtk <tool>"
# (not "rtk <tool>") to preserve the uv venv context. Similarly,
# "uv pip <cmd>" must become "rtk uv pip <cmd>" to preserve uv's pip wrapper.
...
# --- Python tooling ---
  ...
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed -E 's/^uv run ruff /uv run rtk ruff /')"
elif echo "$MATCH_CMD" | grep -qE '^uv[[:space:]]+run[[:space:]]+playwright([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uv run playwright/uv run rtk playwright/')"
elif echo "$MATCH_CMD" | grep -qE '^uvx[[:space:]]+ty[[:space:]]+check([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uvx ty check/rtk err uvx ty check/')"
elif echo "$MATCH_CMD" | grep -qE '^uv[[:space:]]+run[[:space:]]+ty[[:space:]]+check([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uv run ty check/uv run rtk err ty check/')"
elif echo "$MATCH_CMD" | grep -qE '^bandit([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^bandit/rtk err bandit/')"
elif echo "$MATCH_CMD" | grep -qE '^uv[[:space:]]+run[[:space:]]+bandit([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uv run bandit/uv run rtk err bandit/')"
elif echo "$MATCH_CMD" | grep -qE '^uvx[[:space:]]+bandit([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uvx bandit/rtk err uvx bandit/')"
elif echo "$MATCH_CMD" | grep -qE '^uv[[:space:]]+sync([[:space:]]|$)'; then
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uv sync/rtk summary uv sync/')"
  ...
  REWRITTEN="${ENV_PREFIX}$(echo "$CMD_BODY" | sed 's/^uv pip /rtk uv pip /')"
