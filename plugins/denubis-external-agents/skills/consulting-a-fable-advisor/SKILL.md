...
`Write` and `Edit` did not appear in that invocation, and the write attempt returned the same refusal string a correctly restricted pane gives, so a refused write is not evidence the surface is safe. `Glob` and `Grep` were likewise absent, so the provenance greps below would have to route through `Bash`.

Read that enumeration as one dated observation rather than a property of the type. A probe establishes what was available to that invocation; it does not establish what a different dispatch of the same type will carry, which is exactly what the foreground and background split below demonstrates.
